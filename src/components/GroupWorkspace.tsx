import { CalendarClock, ChevronLeft, MapPin, PenLine, RotateCcw, UsersRound } from "lucide-react";
      ...
      <section className="rounded-lg border border-neutral-200 bg-white p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-lg font-semibold text-neutral-900">{formatMeeting(group)}</p>
          </div>
          <div className="text-sm text-neutral-600">
            {topAvailability ? (
              <span>
                현재 최다 가능 시간 {new Date(topAvailability.startsAt).toLocaleString("ko-KR", {
                  day: "numeric",
                  hour: "2-digit",
                  minute: "2-digit",
                  month: "numeric",
                ...
                · {topAvailability.count}명
              </span>
            ) : (
              <span>일정 응답 {responderCount}명</span>
            )}
          </div>
        </div>
      </section>
