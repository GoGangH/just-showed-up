  ...
  return (
    <section className="space-y-3">
      <div className="flex items-end justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-faint">내 그룹</p>
          <h2 className="mt-1 text-xl font-semibold">이번 주 스터디</h2>
        </div>
        <ModalTrigger className="text-sm font-semibold text-muted hover:text-ink" modal="new-group">
          그룹 추가
        </ModalTrigger>
      </div>
      ...
      <div className="grid gap-3">
        {groups.map((group) => {
          ...
          return (
            <PrefetchRouteLink
              className={`block rounded-lg border bg-surface p-5 transition hover:border-line-strong ${
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="inline-flex items-center gap-1 rounded-md bg-line px-2 py-1 text-xs font-semibold text-ink">
                    <Clock3 size={13} />
                    <RemainingTime
                      fallback="모임 시간 미정"
                  ...
                  <p className="mt-2 text-sm text-muted">{formatMeeting(group)}</p>
                </div>
                <div className="min-w-32 text-right text-sm text-muted">
              ...
              <div className="mt-5 flex items-center justify-between gap-4">
                <div className="flex -space-x-2">
                  {group.members.slice(0, 8).map((member) => (
                    <div
                      className={`flex size-9 items-center justify-center rounded-full border-2 border-surface text-xs font-semibold ${
                  ...
                  {group.members.length === 0 ? (
                    <div className="flex size-9 items-center justify-center rounded-full bg-line text-faint">
                      <UsersRound size={15} />
                    </div>
                  ) : null}
                </div>
